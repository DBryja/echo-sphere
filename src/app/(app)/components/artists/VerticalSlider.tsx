  ...
  useEffect(() => {
    if (!sliderRef.current || !barRef.current || !containerRef.current) {
      containerRef.current = document.querySelector(
        ".artists__slider__wrapper",
      );
      sliderRef.current = document.getElementById(
        "artistSlider",
      ) as HTMLDivElement;
      barRef.current = document.querySelector(
        ".artists__slider__bar__container",
      ) as HTMLDivElement;
    }
    ...
    const checkPauseCondition = () => {
      if (shouldPauseRef.current && tlRef.current) {
        ...
        const bonds = [
          nextStepProgress - threshold / 5,
          nextStepProgress + threshold * 3,
        ];
